...
class Supplier extends MX_Controller
{
    ...
    public function cloudsubset_ChecksupplierList(){
    ...
    public function cloudsubset_form($id = null){
        ...
        $data["supplier"] = (object) ($postData = [
            "supplier_id" => $this->input->post("supplier_id", true), "supplier_name" => $this->input->post("supplier_name", true), "mobile" => $this->input->post("supplier_mobile", true), "emailnumber" => $this->input->post("supplier_email", true), "email_address" => $this->input->post("email_address", true), "contact" => $this->input->post("contact", true), "phone" => $this->input->post("phone", true), "fax" => $this->input->post("fax", true), "city" => $this->input->post("city", true), "state" => $this->input->post("state", true), "zip" => $this->input->post("zip", true), "country" => $this->input->post("country", true), "address" => $this->input->post("supplier_address", true), "address2" => $this->input->post("address2", true), "status" => 1,
        ]);
        #-------------------------------#
    ...
    public function cloudsubset_supplier_ledger(){
    ...
    public function cloudsubset_supplier_ledgerData(){
    ...
    $pre_balance = $this->supplier_model->get_opening_balance(
        $supplier_id,
        $start,
        $end
    );
    ...
    public function cloudsubset_supplier_advance(){
    ...
    public function insert_supplier_advance(){
        ...
        $supplier_accledger = [
            "VNo" => $transaction_id, "Vtype" => "Advance", "VDate" => date("Y-m-d"), "COAID" => $supplier_headcode, "Narration" => "supplier Advance For  " . $supplierinfo->supplier_name, "Debit" => !empty($dr) ? $dr : 0, "Credit" => !empty($cr) ? $cr : 0, "IsPosted" => 1, "CreateBy" => $this->session->userdata("id"), "CreateDate" => date("Y-m-d H:i:s"), "IsAppove" => 1,
        ];
        $cc = [
            "VNo" => $transaction_id, "Vtype" => "Advance", "VDate" => date("Y-m-d"), "COAID" => 111000001, "Narration" => "Cash in Hand  For " . $supplierinfo->supplier_name . " Advance", "Debit" => !empty($dr) ? $dr : 0, "Credit" => !empty($cr) ? $cr : 0, "IsPosted" => 1, "CreateBy" => $this->session->userdata("id"), "CreateDate" => date("Y-m-d H:i:s"), "IsAppove" => 1,
        ];
        $this->db->insert("acc_transaction", $supplier_accledger);
        $this->db->insert("acc_transaction", $cc);
        redirect(base_url("supplier_advance_receipt/" .
                    $transaction_id .
                    "/" .
                    $supplier_id));
    }
    ...
    public function cloudsubset_supplier_ledgerinfo($supplier_id){
        $supplier_details = $this->supplier_model->supplier_personal_data($supplier_id);
        $supplier = $this->supplier_model->supplier_list_advance();
        $ledgers = $this->supplier_model->supplier_product_sale_info($supplier_id);
        $data = [
            "title" => display("supplier_ledger"), "ledgers" => $ledgers, "supplier_id" => $supplier_id, "supplier_name" => $supplier_details[0]["supplier_name"], "address" => $supplier_details[0]["address"], "supplier" => $supplier, "links" => "",
        ];
        $data["module"] = "supplier";
        $data["page"] = "supplier_ledger";
        echo Modules::run("template/layout", $data);
