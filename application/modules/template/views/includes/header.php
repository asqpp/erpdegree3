...
<span class="top-fixed-link">
  ...
  <?php if($this->permission1->method('supplier_payment','create')->access()){ ?>
      ...
      <?php } ?>
  <?php if($this->permission1->method('salesman_payment','create')->access()){ ?>
          <a href="<?php echo base_url('salesman_payment')?>" class="btn btn-success btn-outline"><i class="fa fa-money" aria-hidden="true"></i> <?php echo display('salesman_payment')?></a>
      <?php } ?>
