...
class Product_model extends CI_Model {
    ...
    public function salesman_product_list($id){
        $this->db->select('*');
        $this->db->from('salesman_product');
        $this->db->where('product_id', $id);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
    ...
    public function getProductList($postData=null){
         ...
         $searchQuery = "";
         if($searchValue != ''){
            // $searchQuery = " (a.product_name like '%".$searchValue."%' or a.product_model like '%".$searchValue."%' or a.price like'%".$searchValue."%' or c.supplier_price like'%".$searchValue."%' or m.supplier_name like'%".$searchValue."%' or d.salesman_price like'%".$searchValue."%' or n.salesman_name like'%".$searchValue."%') ";
        ...
        //  $this->db->join('salesman_product d','d.product_id = a.product_id','left');
        //  $this->db->join('salesman_information n','n.salesman_id = d.salesman_id','left');
          if($searchValue != '')
        ...
        //  var_dump($records);exit;
         ...
         foreach($records as $record ){
          ...
          $jsaction = "return confirm('Are You Sure ?')";
           ...
           $button .= '<a href="'.$base_url.'product/product/cloudsubset_deleteproduct/'.$record->product_id.'" class="btn btn-xs btn-danger "  onclick="'.$jsaction.'"><i class="fa fa-trash"></i></a>';
         }
      ...
      if($this->permission1->method('manage_product','update')->access()){
        ...
        //  $salesman = '<a href="'.$base_url.'salesman_ledgerinfo/'.$record->salesman_id.'">'.$record->salesman_name.'</a>';
...
public function check_product($id){
    ...
    public function cloudsubset_barcode_productdata($id){
         return $this->db->select('*')
            ->from('product_information')
            ->where('product_id', $id)
            ->get()
     ...
     public function product_purchase_info($product_id) {
        $this->db->select('a.*,b.*,sum(b.quantity) as quantity,sum(b.total_amount) as total_amount,c.supplier_name,d.salesman_name');
        $this->db->from('product_purchase a');
        $this->db->join('product_purchase_details b', 'b.purchase_id = a.purchase_id');
        $this->db->join('supplier_information c', 'c.supplier_id = a.supplier_id');
        $this->db->join('salesman_information d', 'd.salesman_id = a.salesman_id');
        ...
        $this->db->join('invoice_details b', 'b.invoice_id = a.inv_id');
