...
class Product extends MX_Controller
{
    ...
    // category part
    function cloudsubset_category_list()
    {
    ...
    public function cloudsubset_category_form($id = null)
    {
        ...
        #-------------------------------#
        $data['category'] = (object)$postData = ['category_id' => $id, 'category_name' => $this->input->post('category_name', true) , 'status' => $this->input->post('status', true) , ];
        #-------------------------------#
    ...
    public function cloudsubset_deletecategory($id = null)
    {
    ...
    // unit part
    function cloudsubset_unit_list()
    {
    ...
    public function cloudsubset_unit_form($id = null)
    {
        ...
        $this->form_validation->set_rules('status', display('status') , 'max_length[2]');
        #-------------------------------#
        $data['unit'] = (object)$postData = ['unit_id' => $id, 'unit_name' => $this->input->post('unit_name', true) , 'status' => $this->input->post('status', true) , ];
        #-------------------------------#
        if ($this->form_validation->run() === true)
        {
    ...
    public function cloudsubset_deleteunit($id = null)
    {
    ...
    // product part
    public function cloudsubset_product_form($id = null)
    {
        ...
        $data['product'] = (object)$postData = [
                ...
                'unit' => $this->input->post('unit', true) , 'tax' => 0, 
        ...
        'image' => (!empty($image) ? $image : 'my-assets/image/product.png') , 'status' => 1, ];
        ...
        if ($this->form_validation->run() === true)
        {
            ...
            if (empty($id))
            {
                $this->product_model->create_product($postData);
            ...
            else
            {
                ...
                //         $this->db->insert('supplier_product', $supp_prd);
                //     }
                //     $this->db->where('product_id', $id)->delete("salesman_product");
                //     for ($i = 0, $n = count($sa_id);$i < $n;$i++)
                //     {
                //         $sall_id = $sa_id[$i];
                ...
                //         $this->db->insert('salesman_product', $sall_prd);
                //     }
        ...
        else
        {
            ...
            $data['salesman'] = $this->product_model->salesman_list();
            ...
            $data['salesman_pr'] = $this->product_model->salesman_product_list($id);
    ...
    public function cloudsubset_product_list()
    {
    ...
    public function cloudsubset_deleteproduct($id = null)
    {
    ...
    public function cloudsubset_csv_product()
    {
            ...
            if (($handle = fopen($_FILES['upload_csv_file']['tmp_name'], 'r')) !== false)
            {
                ...
                while ($csv_line = fgetcsv($fp, 1024))
                {
                    ...
                    for ($i = 0, $j = count($csv_line);$i < $j;$i++)
                    {
                        ...
                        $insert_csv['salesman_id'] = (!empty($csv_line[7]) ? $csv_line[7] : null);
                        $insert_csv['salesman_price'] = (!empty($csv_line[8]) ? $csv_line[8] : null);
                    }
                    $check_supplier = $this->db->select('*')->from('supplier_information')->where('supplier_name', $insert_csv['supplier_id'])->get()->row();
                    $check_salesman = $this->db->select('*')->from('salesman_information')->where('salesman_name', $insert_csv['salesman_id'])->get()->row();
                    ...
                    if (!empty($check_salesman))
                    {
                        $salesman_id = $check_salesman->salesman_id;
                    }
                    else
                    {
                        $salesmaninfo = array(
                            'salesman_name' => $insert_csv['salesman_id'],
                            'address' => '',
                            'mobile' => '',
                            'details' => '',
                            'status' => 1
                        ...
                        if ($count > 0)
                        {
                            $this->db->insert('salesman_information', $salesmaninfo);
                        }
                        $salesman_id = $this->db->insert_id();
                        $coa = $this->salesman_model->headcode();
                        if ($coa->HeadCode != NULL)
                        {
                            $headcode = $coa->HeadCode + 1;
                        }
                        ...
                        $c_acc = $salesman_id . '-' . $insert_csv['salesman_id'];
                        $createby = $this->session->userdata('id');
                        $createdate = date('Y-m-d H:i:s');
                        ...
                        if ($count > 0)
                        {
                            $this->db->insert('acc_coa', $salesman_coa);
                        }
                    }
                    ...
                    if ($count > 0)
                    {
                        ...
                        $result = $this->db->select('*')->from('product_information')->where('product_name', $data['product_name'])->where('product_model', $data['product_model'])->where('category_id', $category_id)->get()->row();
                        if (empty($result))
                        {
                        ...
                        $splprd = $this->db->select('*')->from('supplier_product')->where('supplier_id', $supplier_id)->where('product_id', $product_id)->get()->num_rows();
                        ...
                        $sall_prd = array(
                            'product_id' => $product_id,
                            'salesman_id' => $salesman_id,
                            'salesman_price' => $insert_csv['salesman_price'],
                            'products_model' => $insert_csv['product_model'],
                        );

                        $splprd = $this->db->select('*')->from('salesman_product')->where('salesman_id', $salesman_id)->where('product_id', $product_id)->get()->num_rows();
                        ...
                        else
                        {
                            ...
                            $this->db->update('salesman_product', $sall_prd);
                        }
    ...
    public function qrgenerator($product_id)
    {
        ...
        $product_info = $this->product_model->cloudsubset_barcode_productdata($product_id);
        $data = array(
    ...
    public function cloudsubset_product_details($product_id = null)
    {
        $details_info = $this->product_model->cloudsubset_barcode_productdata($product_id);
