...
class Accounts_model extends CI_Model
{
    ...
    public function dfs_4($HeadName, $HeadCode, $oResult, $visit, $d)
    {
        ...
        $sopening = 0;
        $balance = $sbalance
            ? number_format($sbalance, 2)
            : number_format(0, 2);
        $opening = $sopening
            ? number_format($sopening, 2)
            : number_format(0, 2);

        if ($d == 0) {
            echo "<li class=\"jstree-open\" >$HeadName <a href=/'javascript:void(0)/' class=\"form-control headanchor\"><span class=\"coa_hd\"><b>Head Name</b></span><span class=\"bal_opening\"><b></b></span><span class=\"bal_span\"><b> </b></span></a>";
        } elseif ($d == 1) {
            echo "<li class=\"jstree-open\" id='" . $HeadCode .  "'><a href='javascript:' class='form-control jstreelip' onclick=\"loadData('" . $HeadCode .  "')\">$HeadName <span class=\"bal_opening\"> </span><span class=\"bal_span_pre\"></span></a>";
        } else {
            echo "<li id='" . $HeadCode .  "'><a href='javascript:' class='form-control'  onclick=\"loadData('" . $HeadCode .  "')\">$HeadName <span class=\"bal_opening\"> </span> <span class=\"bal_span_pre\"></span></a>";
        }
        $p = 0;
        for ($i = 1; $i < count($oResult); $i++) {
            if (!$visit[$i]) {
    ...
    {
        return $data = $this->db->select("*")->from("acc_coa")->where("HeadLevel", 4)->where("IsActive", 1)->where_in("HeadType", ["A", "L"])->order_by("HeadType")->get()->result();
    }

    // Accounts list
    public function Transacc()
    {
        return $data = $this->db->select("*")->from("acc_coa")->where("IsTransaction", 1)->where("IsActive", 1)->order_by("HeadName")->get()->result();
    }

    // Accounts list
    public function getTransationHead()
    {
        return $data = $this->db->select("*")->from("acc_coa")->where("isBankNature", 0)->where("isCashNature", 0)->where("HeadLevel", 4)->where("IsActive", 1)->order_by("HeadName")->get()->result();
    }

    // Accounts list
    public function getCashbankHead()
    {
        return $data = $this->db->select("*")->from("acc_coa")->where("HeadLevel", 4)->where("IsActive", 1)->where("isBankNature", 1)->or_where("isCashNature", 1)->order_by("HeadName")->get()->result();
    }

    // get subcode by subtype
    public function getSubcode($id)
    {
        $subcodes = $this->db->select("*")->from("acc_subcode")->where("subTypeId", $id)->get();
    ...
    {
        $data = $this->db->select("id")->from("acc_transaction")->where("Vtype", $type)->group_by("VNo")->order_by("id", "DESC")->get();
        if ($data->num_rows() > 0) {
    ...
    {
        $data = $this->db->select("av.*, fy.yearName , ac.HeadName, st.subtypeName, sc.name, rc.HeadName as reverseHead" )->from("acc_vaucher av")->join("financial_year fy", "av.fyear = fy.id", "left")->join("acc_coa ac", "av.COAID = ac.HeadCode", "left")->join("acc_coa rc", "av.RevCodde = rc.HeadCode", "left")->join("acc_subtype st", "av.subType = st.id", "left")->join("acc_subcode sc", "av.subCode = sc.id", "left")->where("Vtype", $type)->order_by("av.id", "DESC")->get();
        if ($data->num_rows() > 0) {
            return $data->result();
        }
        return false;
    ...
    {
        $data = $this->db->select("av.*, fy.yearName , ac.HeadName, st.subtypeName, sc.name, rc.HeadName as reverseHead" )->from("acc_vaucher av")->join("financial_year fy", "av.fyear = fy.id", "left")->join("acc_coa ac", "av.COAID = ac.HeadCode", "left")->join("acc_coa rc", "av.RevCodde = rc.HeadCode", "left")->join("acc_subtype st", "av.subType = st.id", "left")->join("acc_subcode sc", "av.subCode = sc.id", "left")->where("av.VDate >=", $start)->where("av.VDate <=", $end)->where("av.isApproved", $status)
            /*->group_by('VNo')  */
->order_by("av.id", "DESC")->get();
        if ($data->num_rows() > 0) {
            return $data->result();
        }
        return false;
    ...
    {
        $data = $this->db->select("VNo, Vtype")->from("acc_vaucher")->where("VDate >=", $start)->where("VDate <=", $end)->where("isApproved", $status)->group_by("VNo")->order_by("id", "ASC")->get();
        if ($data->num_rows() > 0) {
            return $data->result();
        }
        return false;
    ...
    {
        $data = $this->db->select("ob.*, fy.yearName , ac.HeadName, st.subtypeName, sc.name")->from("acc_opening_balance ob")->join("financial_year fy", "ob.fyear = fy.id", "left")->join("acc_coa ac", "ob.COAID = ac.HeadCode", "left")->join("acc_subtype st", "ob.subType = st.id", "left")->join("acc_subcode sc", "ob.subCode = sc.id", "left")->order_by("ob.id", "DESC")->limit($limit, $start)->get();
        if ($data->num_rows() > 0) {
            return $data->result();
        }
        return false;
    ...
    {
        $data = $this->db->select("id")->from("acc_opening_balance")->order_by("id", "DESC")->get();
        if ($data->num_rows() > 0) {
            return $data->num_rows();
        }
        return 0;
    ...
    {
        $data = $this->db->select("ob.*, fy.yearName , ac.HeadName, st.subtypeName, sc.name")->from("acc_opening_balance ob")->join("financial_year fy", "ob.fyear = fy.id", "left")->join("acc_coa ac", "ob.COAID = ac.HeadCode", "left")->join("acc_subtype st", "ob.subType = st.id", "left")->join("acc_subcode sc", "ob.subCode = sc.id", "left")->where("ob.id", $id)->get();
        if ($data->num_rows() > 0) {
            return $data->row();
        }
        return false;
    ...
    {
        $data = $this->db->select("*")->from("financial_year")->order_by("id", "DESC")->limit($limit, $start)->get();
        if ($data->num_rows() > 0) {
            return $data->result();
        }
        return false;
    ...
    {
        $data = $this->db->select("*")->from("financial_year")->where("status", 1)->where("isCloseReq", 0)->limit(1)->order_by("id", "ASC")->get();
        if ($data->num_rows() > 0) {
            return $data->row();
        }
        return false;
    ...
    {
        $data = $this->db->select("*")->from("financial_year")->order_by("status", "DESC")->get();
        if ($data->num_rows() > 0) {
            return $data->num_rows();
        }
        return 0;
    ...
    {
        $data = $this->db->select("*")->from("financial_year")->where("startDate", $sdate)->where("endDate", $edate)->where("yearName", $yn)->get();
        if ($data->num_rows() > 0) {
            return false;
        }
        return true;
    ...
    {
        return $this->db->where("id", $data["id"])->update("acc_subcode", $data);
    }

    // get Subtypes
    public function get_subTypeItems($id)
    {
        $data = $this->db->select("id,name")->from("acc_subcode")->where("subTypeId", $id)->order_by("id", "ASC")->get();
        if ($data->num_rows() > 0) {
            return $data->result();
        }
        return 0;
    ...
    {
        $this->db->where("id", $id)->delete("acc_subcode");
    ...
    {
        return $this->db->where("id", $data["id"])->update("financial_year", $data);
    }

    // Delete financial year
    public function delete_financial_year($id)
    {
        $this->db->where("id", $id)->delete("financial_year");
    ...
    {
        return $data = $this->db->select("*")->from("acc_coa")->like("HeadCode", 1020102, "after")->where("IsTransaction", 1)->order_by("HeadName")->get()->result();
    }

    // Chech sub account is exist
    public function check_child_account($id)
    {
        $data = $this->db->select("HeadCode, HeadLevel")->from("acc_coa")->where("pheadcode", $id)->get();
        if ($data->num_rows() > 0) {
            return $data->result();
        } else {
            return false;
    ...
    public function insert_openingBalance()
    {
        ...
        for ($i = 0; $i < count($dAID); $i++) {
            ...
            $openingbalancetinsert = [
                "fyear" => $fyear, "COAID" => $dbtid, "subType" => $isSubtypev, "subCode" => $subcode, "openDate" => $VDate, "Debit" => $Damnt, "Credit" => $Camnt, "CreateBy" => $CreateBy, "CreateDate" => $createdate, ];
    ...
    {
        $data = $this->db->select("referenceNo")->from("acc_subcode")->where("id", $id)->limit(1)->get();
        if ($data->num_rows() > 0) {
            $val = $data->row();
    ...
    public function insert_debitvoucher()
    {
        ...
        for ($i = 0; $i < count($dAID); $i++) {
            ...
            $debitinsert = [
                "fyear" => $fyear, "VNo" => $vaucherNo, "Vtype" => "DV", "referenceNo" => $refno, "VDate" => $VDate, "COAID" => $dbtid, "Narration" => $Narration, "chequeNo" => $checkno, "chequeDate" => $checkdate, "isHonour" => $ishonour, "ledgerComment" => $Comment, "Debit" => $Damnt, "Credit" => 0.0, "RevCodde" => $reVID, "subType" => $isSubtypev, "subCode" => $subcode, "isApproved" => 0, "CreateBy" => $CreateBy, "CreateDate" => $createdate, "status" => 0, ];
    ...
    public function customer_receive_insert()
    {
        ...
        $paid_amount =
            ($voucher_details ? $voucher_details->paid_amount : 0) +
            ($Credit ? $Credit : 0);
        $due_amount =
            ($voucher_details ? $voucher_details->due_amount : 0) -
            ($Credit ? $Credit : 0);
        $invoice_info = [
    ...
    public function insert_sales_due( $invoice_id = null, $dbtid = null, $amnt_type = null, $amnt = null, $Narration = null, $Comment = null, $reVID = null, $subcode = null ) {
        ...
        $debitinsert = [
            "fyear" => $fyear, "VNo" => $vaucherNo, "Vtype" => "CV", "referenceNo" => $invoice_id, "VDate" => $VDate, "COAID" => $dbtid, "Narration" => $Narration, "ledgerComment" => $Comment, "RevCodde" => $reVID, "subType" => 3, "subCode" => $subcode, "isApproved" => 0, "CreateBy" => $CreateBy, "CreateDate" => $createdate, "status" => 0, ];

        if ($amnt_type == "Debit") {
            $debitinsert["Debit"] = $amnt;
            $debitinsert["Credit"] = 0.0;
        } else {
            $debitinsert["Debit"] = 0.0;
    ...
    {
        $customercode = $this->db->select("*")->from("acc_coa")->where("customer_id", $id)->get()->row();
        $code = $customercode ? $customercode->HeadCode : "";

        $data = $this->db->select("*")->from("invoice")->where("customer_id", $id)->having("due_amount >", 0)->order_by("date", "asc")->get()->result_array();
        ...
        if (empty($data)) {
            ...
            $html .=
                "<select name=\"voucher_no\"   class=\"voucher_no form-control select2\" id=\"voucher_no_1\">";
            $html .= "<option>" . "Select Voucher" . "</option>";

            foreach ($data as $voucher) {
                $html .=
                    "<option value=" .  $voucher["invoice_id"] .  ">" .  $voucher["invoice"] .  "</option>";
            }
    ...
    public function update_debitvoucher()
    {
        ...
        $this->db->where("VNo", $vaucherNo)->delete(" acc_vaucher");
        $fyear = $this->input->post("fyear", true);
        ...
        for ($i = 0; $i < count($dAID); $i++) {
            ...
            $debitinsert = [
                "fyear" => $fyear, "VNo" => $vaucherNo, "Vtype" => "DV", "referenceNo" => $refno, "VDate" => $VDate, "COAID" => $dbtid, "Narration" => $Narration, "chequeNo" => $checkno, "chequeDate" => $checkdate, "isHonour" => $ishonour, "ledgerComment" => $Comment, "Debit" => $Damnt, "Credit" => 0.0, "RevCodde" => $reVID, "subType" => $isSubtypev, "subCode" => $subcode, "isApproved" => 0, "CreateBy" => $CreateBy, "CreateDate" => $CreateDate, "UpdateBy" => $updatedBy, "UpdateDate" => $updatedDate, "status" => 0, ];
    ...
    {
        return $data = $this->db->select("VNo as voucher")->from("acc_transaction")->like("VNo", "DV-", "after")->order_by("ID", "desc")->get()->row();
    }

    public function Cashvoucher()
    {
        return $data = $this->db->select("VNo as voucher")->from("acc_transaction")->like("VNo", "CHV-", "after")->order_by("ID", "desc")->get()->row();
    }
    // balance Adjustment voucher
    public function balanceadjvoucher()
    {
        return $data = $this->db->select("VNo as voucher")->from("acc_transaction")->like("VNo", "BLA-", "after")->order_by("ID", "desc")->get()->row();
    }

    // bank voucher
    public function bankvoucher()
    {
        return $data = $this->db->select("VNo as voucher")->from("acc_transaction")->like("VNo", "BAD-", "after")->order_by("ID", "desc")->get()->row();
    }

    // Credit voucher no
    public function crVno()
    {
        return $data = $this->db->select("VNo as voucher")->from("acc_transaction")->like("VNo", "CV-", "after")->order_by("ID", "desc")->get()->row();
    }
    ...
    {
        return $data = $this->db->select("Max(VNo) as voucher")->from("acc_transaction")->like("VNo", "Contra-", "after")->order_by("ID", "desc")->get()->result_array();
    }
    ...
    public function insert_creditvoucher()
    {
        ...
        for ($i = 0; $i < count($dAID); $i++) {
            ...
            $creditinsert = [
                "fyear" => $fyear, "VNo" => $vaucherNo, "Vtype" => "CV", "referenceNo" => $refno, "VDate" => $VDate, "COAID" => $dbtid, "Narration" => $Narration, "chequeNo" => $checkno, "chequeDate" => $checkdate, "isHonour" => $ishonour, "ledgerComment" => $Comment, "Debit" => 0.0, "Credit" => $Camnt, "RevCodde" => $reVID, "subType" => $isSubtypev, "subCode" => $subcode, "isApproved" => 0, "CreateBy" => $CreateBy, "CreateDate" => $createdate, "status" => 0, ];
    ...
    public function insert_contravoucher()
    {
        ...
        for ($i = 0; $i < count($dAID); $i++) {
            ...
            $Comment = $txtComment[$i];
            $contrainsert = [
                "fyear" => $fyear, "VNo" => $vaucherNo, "Vtype" => "CT", "referenceNo" => null, "VDate" => $VDate, "COAID" => $dbtid, "Narration" => $Narration, "chequeNo" => null, "chequeDate" => null, "isHonour" => 0, "ledgerComment" => $Comment, "Debit" => $Damnt, "Credit" => $Camnt, "RevCodde" => $reVID, "subType" => 1, "subCode" => null, "isApproved" => 0, "CreateBy" => $CreateBy, "CreateDate" => $createdate, "status" => 0, ];
    ...
    public function insert_journalvoucher()
    {
        ...
        for ($i = 0; $i < count($dAID); $i++) {
            ...
            $jurnalinsert = [
                "fyear" => $fyear, "VNo" => $vaucherNo, "Vtype" => "JV", "referenceNo" => $refno, "VDate" => $VDate, "COAID" => $dbtid, "Narration" => $Narration, "chequeNo" => null, "chequeDate" => null, "isHonour" => 0, "ledgerComment" => $Comment, "Debit" => $Damnt, "Credit" => $Camnt, "RevCodde" => $reAccount, "subType" => $isSubtypev, "subCode" => $subcode, "isApproved" => 0, "CreateBy" => $CreateBy, "CreateDate" => $createdate, "status" => 0, ];
    ...
    {
        return $data = $this->db->select("Max(VNo) as voucher")->from("acc_transaction")->like("VNo", "Journal-", "after")->order_by("id", "desc")->get()->result_array();
    }

    //Get all Bank name
    public function get_all_bank_list()
    {
        return $data = $this->db->select("HeadCode, HeadName")->from("acc_coa")->where("isBankNature", 1)->order_by("id", "ASC")->get()->result();
    }
    ...
    public function reconciliation_all($data)
    {
        $udata = [
            "isHonour" => 1, ];
        $uptransation = $this->db->where_in("VNo", $data)->update("acc_transaction", $udata);
        $upvoucher = $this->db->where_in("VNo", $data)->update("acc_vaucher", $udata);
    ...
    public function approve_voucher()
    {
        ...
        return $approveinfo = $this->db->select("acc_vaucher.id as vid,acc_vaucher.VNo,acc_vaucher.referenceNo,acc_vaucher.Narration,sum(Credit) as Credit,sum(Debit) as Debit, acc_coa.HeadName" )->from("acc_vaucher")->join("acc_coa", "acc_vaucher.COAID = acc_coa.HeadCode", "left")->where_in("Vtype", $values)->where("isApproved", 0)->group_by("VNo")->order_by("referenceNo")->get()->result_array();
    }
    ...
    public function getvoucherList($postData = null)
    {
        ...
        if ($searchValue != "") {
            $searchQuery =
                " (acc_vaucher.referenceNo like '%" . $searchValue .  "%' or acc_vaucher.VNo like '%" . $searchValue .  "%')";
        }

        ## Total number of records without filtering

        $this->db->select("acc_vaucher.id")->from("acc_vaucher")->join("acc_coa", "acc_vaucher.COAID = acc_coa.HeadCode", "left")->where_in("Vtype", $values)->where("isApproved", 0)->group_by("VNo");
        ...
        ## Fetch records
        $this->db->select("acc_vaucher.id as vid,acc_vaucher.VNo,acc_vaucher.referenceNo,acc_vaucher.Narration,sum(Credit) as Credit,sum(Debit) as Debit, acc_coa.HeadName" )->from("acc_vaucher")->join("acc_coa", "acc_vaucher.COAID = acc_coa.HeadCode", "left")->where_in("Vtype", $values)->where("isApproved", 0)->group_by("VNo");
        ...
        foreach ($records as $record) {
            ...
            $checkbox =
                '<input type="checkbox" class="approvalCheckbox" value="' . $record->VNo .
                '" name="vapprove[]"/> &nbsp; ' . $sl .  " ";

            $data[] = [
                "checkbox" => $checkbox, "referenceNo" => $record->referenceNo, "VNo" => $record->VNo, "HeadName" => $record->HeadName, "Narration" => $record->Narration, "Debit" => $record->Debit, "Credit" => $record->Credit, "button" => $button, ];
            $sl++;
        }

        ## Response
        $response = [
            "draw" => intval($draw), "iTotalRecords" => $totalRecordwithFilter, "iTotalDisplayRecords" => $totalRecords, "aaData" => $data, ];
    ...
    public function getdebitvoucherList($postData = null)
    {
        ...
        if ($searchValue != "") {
            $searchQuery =
                " (av.VDate like '%" . $searchValue .  "%' or av.VNo like '%" . $searchValue .  "%')";
        }

        ## Total number of records without filtering

        $this->db->select("av.id")->from("acc_vaucher av")->join("financial_year fy", "av.fyear = fy.id", "left")->join("acc_coa ac", "av.COAID = ac.HeadCode", "left")->join("acc_coa rc", "av.RevCodde = rc.HeadCode", "left")->join("acc_subtype st", "av.subType = st.id", "left")->join("acc_subcode sc", "av.subCode = sc.id", "left")->where("Vtype", "DV");
        ...
        ## Fetch records
        $this->db->select("av.VNo,av.VDate,av.ledgerComment, av.Debit, av.Credit,av.Vtype,av.isApproved ,av.id,av.isyearClosed, fy.yearName , ac.HeadName, st.subtypeName, sc.name, rc.HeadName as reverseHead" )->from("acc_vaucher av")->join("financial_year fy", "av.fyear = fy.id", "left")->join("acc_coa ac", "av.COAID = ac.HeadCode", "left")->join("acc_coa rc", "av.RevCodde = rc.HeadCode", "left")->join("acc_subtype st", "av.subType = st.id", "left")->join("acc_subcode sc", "av.subCode = sc.id", "left")->where("Vtype", "DV");
        ...
        foreach ($records as $record) {
            ...
            $button .=
                ' <a href="javascript:" class="btn btn-xs" onClick="return showVaucherDetail(\'' . $record->VNo .
                '\',\'' . $record->Vtype .
                '\')" title="View Vaucher"><i class="fa fa-eye"></i></a>';

            if ($this->permission1->method("debit_voucher", "update")->access() ):
                if ($record->isApproved == 0) {
                    $button .=
                        '<a href="' .     $base_url .     "edit_voucher/" .     $record->id .     '" class="btn btn-xs btn-success" title="Edit Vaucher"><i class="fa fa-pencil"></i></a>';
                } else {
                    if ($record->isyearClosed == 0) {
                        $button .=
                            '<button class="btn btn-xs btn-success" onclick="return reverseVoucher(\'' .         $record->VNo .         '\')" title="Reverse Vaucher"><i class="fa fa-undo"></i></button>';
                    }
                }
            endif;
            ...
            if ($this->permission1->method("debit_voucher", "delete")->access() ):
                if ($record->isApproved == 0) {
                    $button .=
                        ' <button class="btn btn-xs btn-danger"  onclick="return deleteVoucher(\'' .     $record->VNo .     '\')" title="Delete Vaucher"><i class="fa fa-trash"></i></button>';
                }
            endif;

            $data[] = [
                "sl" => $sl, "VNo" => $record->VNo, "VDate" => $record->VDate, "HeadName" => $record->HeadName, "ledgerComment" => $record->ledgerComment, "name" => $record->name, "Debit" => $record->Debit, "Credit" => $record->Credit, "reverseHead" => $record->reverseHead
                    ? $record->reverseHead
                    : "", "button" => $button, ];
            $sl++;
        }
    ...
    public function getcreditvoucherList($postData = null)
    {
        ...
        $this->db->select("av.id")->from("acc_vaucher av")->join("financial_year fy", "av.fyear = fy.id", "left")->join("acc_coa ac", "av.COAID = ac.HeadCode", "left")->join("acc_coa rc", "av.RevCodde = rc.HeadCode", "left")->join("acc_subtype st", "av.subType = st.id", "left")->join("acc_subcode sc", "av.subCode = sc.id", "left")->where("Vtype", "CV");
        ...
        ## Fetch records
        $this->db->select("av.VNo,av.VDate,av.ledgerComment, av.Debit, av.Credit,av.Vtype,av.isApproved ,av.id,av.isyearClosed, fy.yearName , ac.HeadName, st.subtypeName, sc.name, rc.HeadName as reverseHead" )->from("acc_vaucher av")->join("financial_year fy", "av.fyear = fy.id", "left")->join("acc_coa ac", "av.COAID = ac.HeadCode", "left")->join("acc_coa rc", "av.RevCodde = rc.HeadCode", "left")->join("acc_subtype st", "av.subType = st.id", "left")->join("acc_subcode sc", "av.subCode = sc.id", "left")->where("Vtype", "CV");
    ...
    public function getjournalvoucherList($postData = null)
    {
        ...
        $this->db->select("av.id")->from("acc_vaucher av")->join("financial_year fy", "av.fyear = fy.id", "left")->join("acc_coa ac", "av.COAID = ac.HeadCode", "left")->join("acc_coa rc", "av.RevCodde = rc.HeadCode", "left")->join("acc_subtype st", "av.subType = st.id", "left")->join("acc_subcode sc", "av.subCode = sc.id", "left")->where("Vtype", "JV");
        ...
        ## Fetch records
        $this->db->select("av.VNo,av.VDate,av.ledgerComment, av.Debit, av.Credit,av.Vtype,av.isApproved ,av.id,av.isyearClosed, fy.yearName , ac.HeadName, st.subtypeName, sc.name, rc.HeadName as reverseHead" )->from("acc_vaucher av")->join("financial_year fy", "av.fyear = fy.id", "left")->join("acc_coa ac", "av.COAID = ac.HeadCode", "left")->join("acc_coa rc", "av.RevCodde = rc.HeadCode", "left")->join("acc_subtype st", "av.subType = st.id", "left")->join("acc_subcode sc", "av.subCode = sc.id", "left")->where("Vtype", "JV");
        ...
        foreach ($records as $record) {
            ...
            $data[] = [
                "sl" => $sl, "VNo" => $record->VNo, "VDate" => $record->VDate, "HeadName" => $record->HeadName, "ledgerComment" => $record->ledgerComment, "Debit" => $record->Debit, "Credit" => $record->Credit, "reverseHead" => $record->reverseHead? $record->reverseHead : "", "button" => $button, ];
            $sl++;
        }
    ...
    {
        return $vou_info = $this->db->select("*")->from("acc_transaction")->where("VNo", $id)->get()->result_array();
    }

    public function approved($data = [])
    {
        return $this->db->where("VNo", $data["VNo"])->update("acc_transaction", $data);
    }
    // Approved Vaucher
        ...
        if ($vaucherdata) {
            foreach ($vaucherdata as $row) {
                ...
                if ($instran) {
                    ...
                    $revercetransationinsert = [
                        "vid" => $row->id,         "fyear" => $row->fyear,         "VNo" => $row->VNo,         "Vtype" => $row->Vtype,         "referenceNo" => $row->referenceNo,         "VDate" => $row->VDate,         "COAID" => $row->RevCodde,         "Narration" => $row->Narration,         "chequeNo" => !empty($row->chequeNo)
                            ? $row->chequeNo
                            : "",         "chequeDate" => $row->chequeDate,         "isHonour" => $row->isHonour,         "ledgerComment" => $row->ledgerComment,         "Debit" => $row->Credit,         "Credit" => $row->Debit,         "StoreID" => 0,         "IsPosted" => 1,         "RevCodde" => $row->COAID,         "subType" => $row->subType,         "subCode" => $row->subCode,         "IsAppove" => 1,         "CreateBy" => $ApprovedBy,         "CreateDate" => $approvedDate,     ];
    ...
    {
        return $vou_info = $this->db->select("*")->from("acc_vaucher")->where("VNo", $id)->get()->result();
    }
    //debit update voucher
    public function dbvoucher_updata($id)
    {
        return $vou_info = $this->db->select("*")->from("acc_transaction")->where("VNo", $id)->where("Credit <", 1)->get()->result();
    }

    //credit voucher update
    public function crdtvoucher_updata($id)
    {
        return $vou_info = $this->db->select("*")->from("acc_transaction")->where("VNo", $id)->where("Debit <", 1)->get()->result();
    }
    ...
    public function update_journalvoucher()
    {
        ...
        for ($i = 0; $i < count($dAID); $i++) {
            ...
            $jurnalinsert = [
                "fyear" => $fyear, "VNo" => $vaucherNo, "Vtype" => "JV", "referenceNo" => $refno, "VDate" => $VDate, "COAID" => $dbtid, "Narration" => $Narration, "ledgerComment" => $Comment, "Debit" => $Damnt, "Credit" => $Camnt, "RevCodde" => $reAccount, "subType" => $isSubtypev, "subCode" => $subcode, "isApproved" => 0, "UpdateBy" => $updatedBy, "UpdateDate" => $updatedDate, "status" => 0, ];
    ...
    public function update_contravoucher()
    {
        ...
        for ($i = 0; $i < count($dAID); $i++) {
            ...
            $Comment = $txtComment[$i];
            $contrainsert = [
                "fyear" => $fyear, "VNo" => $vaucherNo, "Vtype" => "CT", "referenceNo" => null, "VDate" => $VDate, "COAID" => $dbtid, "Narration" => $Narration, "ledgerComment" => $Comment, "Debit" => $Damnt, "Credit" => $Camnt, "RevCodde" => $reVID, "subType" => 1, "subCode" => null, "isApproved" => 0, "UpdateBy" => $updatedBy, "UpdateDate" => $updatedDate, "status" => 0, ];
    ...
    {
        return $cr_info = $this->db->select("*")->from("acc_transaction")->where("VNo", $id)->where("Credit<", 1)->get()->row();
    }
    // debit update voucher credit info
    public function crvoucher_updata($id)
    {
        return $v_info = $this->db->select("*")->from("acc_transaction")->where("VNo", $id)->where("Debit<", 1)->get()->row();
    }
    ...
    public function update_creditvoucher()
    {
        ...
        for ($i = 0; $i < count($dAID); $i++) {
            ...
            $creditinsert = [
                "fyear" => $fyear, "VNo" => $vaucherNo, "Vtype" => "CV", "referenceNo" => $refno, "VDate" => $VDate, "COAID" => $dbtid, "Narration" => $Narration, "chequeNo" => $checkno, "chequeDate" => $checkdate, "isHonour" => $ishonour, "ledgerComment" => $Comment, "Debit" => 0.0, "Credit" => $Camnt, "RevCodde" => $reVID, "subType" => $isSubtypev, "subCode" => $subcode, "isApproved" => 0, "CreateBy" => $CreateBy, "CreateDate" => $CreateDate, "UpdateBy" => $updatedBy, "UpdateDate" => $updatedDate, "status" => 0, ];
    ...
    public function trial_balance_report($FromDate, $ToDate, $WithOpening)
    {
        ...
        $sql =
            "SELECT * FROM acc_coa WHERE IsGL=1 AND IsActive=1 AND HeadType IN ('A','L') ORDER BY HeadCode";
        $oResultTr = $this->db->query($sql);

        $sql =
            "SELECT * FROM acc_coa WHERE IsGL=1 AND IsActive=1 AND HeadType IN ('I','E') ORDER BY HeadCode";
        $oResultInEx = $this->db->query($sql);

        $data = [
            "oResultTr" => $oResultTr->result_array(), "oResultInEx" => $oResultInEx->result_array(), "WithOpening" => $WithOpening, ];
    ...
    {
        return $oldyear = $this->db->select("id,yearName")->from("financial_year")->where("status", 0)->order_by("endDate", "DESC")->get()->result();
    }
    ...
    {
        return $chac = $this->db->select("HeadCode,HeadName")->from("acc_coa")->where("HeadCode", $cmbGLCode)->get()->row();
    }

    public function transational_charteraccount2($cmbGLCode)
    {
        $chac = $this->db->select("HeadName")->from("acc_coa")->where("HeadCode", $cmbGLCode)->get()->row();
    ...
    {
        $CharterAccounts = $this->db->select("HeadCode,HeadName")->from("acc_coa")->where("HeadType", $type)->where("pheadcode", $phead)->get()->result();
        return $CharterAccounts;
    }
    //get Acccount head by assetcode
    public function get_charter_accounts_by_assetCode($type, $assetCode)
    {
        $CharterAccounts = $this->db->select("HeadCode,HeadName,pheadcode, assetCode, depCode,DepreciationRate" )->from("acc_coa")->where("depCode", $assetCode)->where("HeadType", $type)->get()->row();
        if ($CharterAccounts) {
            return $CharterAccounts;
        } else {
            return false;
            ...
            foreach ($secondLevel as $chac) {
                ...
                if ($thirdLevel) {
                    ...
                    foreach ($thirdLevel as $tdl) {
                        ...
                        $cdata = [
                            "headCode" => $tdl->HeadCode, "headName" => $tdl->HeadName, "amount" => $balance,         ];
                        array_push($innerArray, $cdata);
                    }
                }
                $data = [ "headCode" => $chac->HeadCode, "headName" => $chac->HeadName, "subtotal" => $subTotal, "innerHead" => $innerArray, ];
                array_push($secondArray, $data);
            }
        }
        $maina = [
            "head" => $phead, "gtotal" => $sumTotal, "nextlevel" => $secondArray, ];
    ...
    {
        $statements = $this->db->select("*")->from("acc_monthly_balance")->where("COAID", $head)->where("fyear", $fyear)->get()->row();
        if ($statements) {
            return $statements;
        } else {
            return false;
            ...
            foreach ($secondLevel as $chac) {
                ...
                if ($thirdLevel) {
                    ...
                    foreach ($thirdLevel as $tdl) {
                        ...
                        $cdata = [
                            "headCode" => $tdl->HeadCode, "headName" => $tdl->HeadName, "amount1" => $balance1, "amount2" => $balance2, "amount3" => $balance3, "amount4" => $balance4, "amount5" => $balance5, "amount6" => $balance6, "amount7" => $balance7, "amount8" => $balance8, "amount9" => $balance9, "amount10" => $balance10, "amount11" => $balance11, "amount12" => $balance12,         ];
                        array_push($innerArray, $cdata);
                    }
                }
                $data = [ "headCode" => $chac->HeadCode, "headName" => $chac->HeadName, "subtotal" => $subTotal1, "subtota2" => $subTotal2, "subtota3" => $subTotal3, "subtota4" => $subTotal4, "subtota5" => $subTotal5, "subtota6" => $subTotal6, "subtota7" => $subTotal7, "subtota8" => $subTotal8, "subtota9" => $subTotal9, "subtotal0" => $subTotal10, "subtotal1" => $subTotal11, "subtotal2" => $subTotal12, "innerHead" => $innerArray, ];
                array_push($secondArray, $data);
            }
        }
        $maina = [
            "head" => $phead, "gtotal1" => $sumTotal1, "gtotal2" => $sumTotal2, "gtotal3" => $sumTotal3, "gtotal4" => $sumTotal4, "gtotal5" => $sumTotal5, "gtotal6" => $sumTotal6, "gtotal7" => $sumTotal7, "gtotal8" => $sumTotal8, "gtotal9" => $sumTotal9, "gtotal10" => $sumTotal10, "gtotal11" => $sumTotal11, "gtotal12" => $sumTotal12, "nextlevel" => $secondArray, ];
    ...
    public function get_from_secondlevel_expenses($type, $hCode, $fyear)
    {
        ...
        if ($thirdLevel) {
            foreach ($thirdLevel as $tdl) {
                ...
                $cdata = [ "headCode" => $tdl->HeadCode, "headName" => $tdl->HeadName, "amount1" => $balance1, "amount2" => $balance2, "amount3" => $balance3, "amount4" => $balance4, "amount5" => $balance5, "amount6" => $balance6, "amount7" => $balance7, "amount8" => $balance8, "amount9" => $balance9, "amount10" => $balance10, "amount11" => $balance11, "amount12" => $balance12, ];
                array_push($innerArray, $cdata);
            }
        }
        $data = [
            "headCode" => $hCode, "headName" => $phead->HeadName, "subtota1" => $subTotal1, "subtota2" => $subTotal2, "subtota3" => $subTotal3, "subtota4" => $subTotal4, "subtota5" => $subTotal5, "subtota6" => $subTotal6, "subtota7" => $subTotal7, "subtota8" => $subTotal8, "subtota9" => $subTotal9, "subtota10" => $subTotal10, "subtota11" => $subTotal11, "subtota12" => $subTotal12, "innerHead" => $innerArray, ];
        array_push($secondArray, $data);
    ...
    public function get_fixedasset_report($type, $hCode, $fyear)
    {
        ...
        if ($thirdLevel) {
            ...
            foreach ($thirdLevel as $tdl) {
                ...
                if ($transationLevel) {
                    foreach ($transationLevel as $trans) {
                        ...
                        if ($trans->DepreciationRate != 0) {
                            $ebalence +=
                                $dbalence * ($trans->DepreciationRate / 100);
                        ...
                        $trdata = [
                            "headCode" => $trans->HeadCode, "headName" => $trans->HeadName, "openig" => $abalence, "curentDebit" => $bbalence, "curentCredit" => $cbalence, "curentValue" => $dbalence, "depRate" => $deprate, "depAmount" => $ebalence, "revOpening" => $fbalence, "revCredit" => $gbalence, "revDebit" => $hbalence, "revBalance" => $ibalence, "famount" => $jbalence,         ];
                ...
                $cdata = [ "headCode" => $tdl->HeadCode, "headName" => $tdl->HeadName, "openig" => $openig, "curentDebit" => $curentDebit, "curentCredit" => $curentCredit, "curentValue" => $curentValue, "depAmount" => $depAmount, "revOpening" => $revOpening, "revCredit" => $revCredit, "revDebit" => $revDebit, "revBalance" => $revBalance, "famount" => $famount, "innerHead" => $transArray, ];
                array_push($innerArray, $cdata);
            }
        }
        $data = [
            "headCode" => $hCode, "headName" => $phead->HeadName, "subtotal1" => $subTotal1, "subtotal2" => $subTotal2, "subtotal3" => $subTotal3, "subtotal4" => $subTotal4, "subtotal5" => $subTotal5, "subtotal6" => $subTotal6, "subtotal7" => $subTotal7, "subtotal8" => $subTotal8, "subtotal9" => $subTotal9, "subtotal10" => $subTotal10, "nextlevel" => $innerArray, ];
        array_push($secondArray, $data);
        return $secondArray;
    }
    ...
    public function get_previous_financial_year($numYear)
    {
        ...
        for ($i = 1; $i <= $numYear; $i++) {
            ...
            $fyear = $this->db
                ->select("yearName")
                ->from("financial_year")
                ->where("startDate", $previousStartDate)
                ->where("endDate", $previousEnddate)
                ->get()
                ->row();
            if ($fyear) {
        ...
        if (isset($fyear->id)) {
            ...
            if ($oldbalanced) {
                if ($type == "A") {
                    ...
                    } elseif ($oldbalanced->Credit > $oldbalanced->Debit) {
            ...
            foreach ($secondLevel as $chac) {
                ...
                if ($thirdLevel) {
                    ...
                    foreach ($thirdLevel as $tdl) {
                        ...
                        $cdata = [
                            "headCode" => $tdl->HeadCode, "headName" => $tdl->HeadName, "amount" => $balance, "secondyear" => $secondbalance, "thirdyear" => $thirdbalance, "fourthyear" => $fourthbalance,         ];
                        array_push($innerArray, $cdata);
                    }
                }
                $data = [ "headCode" => $chac->HeadCode, "headName" => $chac->HeadName, "subtotal" => $subTotal, "ssubtotal" => $subTotal1, "tsubtotal" => $subTotal2, "fsubtotal" => $subTotal3, "innerHead" => $innerArray, ];
                array_push($secondArray, $data);
            }
        }
        $maina = [
            "head" => $phead, "gtotal" => $sumTotal, "sgtotal" => $sumTotal1, "tgtotal" => $sumTotal2, "fgtotal" => $sumTotal3, "nextlevel" => $secondArray, ];
                        ...
                        if ($transationLevel) {
                            foreach ($transationLevel as $trans) {
                                ...
                                if ($trans->subType == 1) {
                                    ...
                                    $openingbalancetinsert = ["fyear" => $fyear, "COAID" => $returnarning->LPLCode, "subType" => 1, "subCode" => null, "openDate" => $openDate, "Debit" => $Damnt, "Credit" => $Camnt, "CreateBy" => $CreateBy, "CreateDate" => $createdate, ];
                                    ...
                                    if ($subcodes) {
                                        foreach ($subcodes as $subcode) {
                                            ...
                                            $openingbalancetinsert = [ "fyear" => $fyear, "COAID" => $returnarning->LPLCode, "subType" => $trans->subType, "subCode" => $subcode->id, "openDate" => $openDate, "Debit" => $Damnt, "Credit" => $Camnt, "CreateBy" => $CreateBy, "CreateDate" => $createdate,     ];
                                            $openin = $this->insert_openingBalance_by_closing($openingbalancetinsert);
                                        }
                                    }
                                }
    ...
    private function get_all_transational_voucher( $reportType, $startDate, $endDate, $vType ) {
        ...
        if ($vType == "DV") {
            $this->db->where("Debit !=", 0.0);
        } else {
            $this->db->where("Credit !=", 0.0);
        }
                ...
                if ($paymentHead) {
                    ...
                    foreach ($paymentHead as $payhead) {
                        ...
                        if ($headBalance) {
                            ...
                            $darray = [
                                "code" => $payhead->HeadCode, "headName" => $payhead->HeadName, "debit" => $dbalance, "credit" => $cbalance,             ];
                            array_push($paymentheadarray, $darray);
                        }
                    }
                    if (count($paymentheadarray) > 0) {
                        $parray = [
                            "hcode" => $third->HeadCode, "headName" => $third->HeadName, "innerHead" => $paymentheadarray, "subtotal" => $subtotal,         ];
    ...
    public function store_transation_summery($coaid, $date)
    {
        ...
        $data = [
            "fyear" => $fyear, "COAID" => $coaid, "balance" . $curentmonth => $summery, "updatedDate" => date("Y-m-d h:i:s"), ];
    ...
    {
        $vaucherdata = $this->db->select("*")->from("acc_vaucher")->where("VNo", $vno)->get()->result();
        $this->db->where("VNo", $vno);
        $del = $this->db->delete("acc_transaction");
        $updatedBy = $this->session->userdata("id");
        $updateddate = date("Y-m-d H:i:s");
        $uparray = [
            "isApproved" => 0, "status" => 0, "UpdateBy" => $updatedBy, "UpdateDate" => $updateddate, ];
        if ($del) {
            $this->db->where("VNo", $vno);
    ...
    {
        $result = $this->db->select("HeadCode,HeadName")->from("acc_coa")->where("subType", $id)->get();
        if ($result->num_rows() > 0) {
            return $result->result();
        } else {
            return false;
    ...
    public function profit_loss_serach()
    {
        ...
        $data = [
            "oResultAsset" => $sql1->result(), "oResultLiability" => $sql2->result(), ];
        return $data;
    }
    ...
    {
        $data = $this->db->select("*")->from("acc_coa")->where("HeadCode", $id)->get()->row();
        return $data;
    }
    ...
    {
        return $data = $this->db->select("VNo as voucher")->from("acc_transaction")->like("VNo", "PM-", "after")->order_by("ID", "desc")->get()->result_array();
    }
    // customer code
    public function Creceive()
    {
        return $data = $this->db->select("VNo as voucher")->from("acc_transaction")->like("VNo", "CR-", "after")->order_by("ID", "desc")->get()->result_array();
    }
    ...
    public function insert_cashadjustment()
    {
        ...
        $createdate = date("Y-m-d H:i:s");
        $fyear = $this->db->select("yearName")->from("financial_year")->where("status", 1)->limit(1)->get()->row();

        $cc = [
            "VNo" => $voucher_no, "fyear" => $fyear->yearName, "Vtype" => $Vtype, "VDate" => $VDate, "COAID" => 1020101, "Narration" => "Cash Adjustment ", "Debit" => $debit, "Credit" => $credit, "IsPosted" => 1, "CreateBy" => $CreateBy, "CreateDate" => $createdate, "IsAppove" => 1, ];
    ...
    public function insert_bankadjustment()
    {
        ...
        $cc = [
            "VNo" => $voucher_no, "fyear" => $fyear->yearName, "Vtype" => $Vtype, "VDate" => $VDate, "COAID" => $headcode, "Narration" => $Narration, "Debit" => $debit, "Credit" => $credit, "IsPosted" => 1, "CreateBy" => $CreateBy, "CreateDate" => $createdate, "IsAppove" => 1, ];
    ...
    *--------------------------*/
    ...
    {
        $query = $this->db->select("trans.*, coa.HeadName as dbtcrdHead, rc.HeadName as xy,  CONCAT_WS(' ', emp1.first_name, emp1.last_name) as created_by" )->from("acc_vaucher trans")->join("acc_coa rc", "rc.HeadCode = trans.COAID", "left")->join("acc_coa coa", "coa.HeadCode=trans.RevCodde", "left")->join("users emp1", "emp1.id=trans.CreateBy", "left")->where("trans.VNo", $VNo)->get()->row();

        if (!empty($query)) {
            $query->details = $this->db
                ->select("trans1.*, acc_coa.HeadName,rv.HeadName as rHead, acc_subcode.name as subname")
    ...
    {
        return $this->db->select("*")->from("supplier_information")->where("supplier_id", $supplier_id)->get()->result_array();
    }

    public function supplierpaymentinfo($voucher_no, $purchase_date)
    {
        return $this->db->select("acc_vaucher.*,SUM(acc_vaucher.Debit) as sumDebit, product_purchase.chalan_no" )->from("acc_vaucher")->join("product_purchase", "acc_vaucher.referenceNo=product_purchase.purchase_id", "left" )->where("acc_vaucher.referenceNo", $voucher_no)->where("acc_vaucher.CreateDate", $purchase_date)->get()->result_array();
    }

    public function banklist()
    {
        return $this->db->select("*")->from("bank_information")->order_by("bank_name", "asc")->get()->result_array();
    }
    ...
    public function insert_balanceadjustment()
    {
        ...
        $cc = [
            "VNo" => $voucher_no, "fyear" => $fyear->yearName, "Vtype" => $Vtype, "VDate" => $VDate, "COAID" => !empty($childheadcodes)
                ? $childheadcodes
                : $parentheadcode, "Narration" => $Narration, "Debit" => $debit, "Credit" => $credit, "IsPosted" => 1, "CreateBy" => $CreateBy, "CreateDate" => $createdate, "IsAppove" => 1, ];
    ...
    {
        return $this->db->select("*")->from("acc_coa")->where("PHeadName", "Assets")->get()->result_array();
    }
    public function liabilities_data()
    {
        return $this->db->select("*")->from("acc_coa")->where("PHeadName", "Liabilities")->get()->result_array();
    }
    public function equity()
    {
        return $this->db->select("*")->from("acc_coa")->where("PHeadName", "Equity")->get()->result_array();
    }
    public function income_fields()
    {
        return $this->db->select("*")->from("acc_coa")->where("PHeadName", "Income")->get()->result_array();
    }
    public function expense_fields()
    {
        return $this->db->select("*")->from("acc_coa")->where("PHeadName", "Expence")->get()->result_array();
    }
    ...
    {
        $data = $this->db->select("id")->from("financial_year")->order_by("id", "DESC")->where("isCloseReq", 1)->limit(1)->get();
        if ($data->num_rows() > 0) {
            $row = $data->row();
            return $row->id;
        }
    ...
    {
        $data = $this->db->select("id")->from("financial_year")->order_by("id", "DESC")->where("status", 1)->limit(1)->get();
        if ($data->num_rows() > 0) {
            return $data->row();
        }
        return false;
    ...
    {
        return $data = $this->db->select("*")->from("acc_coa")->where("id", $id)->get()->row();
    }

    public function payment_methodlist()
    {
        return $data = $this->db->select("*")->from("acc_coa")->where("PHeadName", "Cash at Bank")->get()->result();
    }
    ...
    {
        $supplierhcode = $this->db->select("*")->from("acc_coa")->where("supplier_id", $id)->get()->row();
        $code = $supplierhcode ? $supplierhcode->HeadCode : "";

        $data = $this->db->select("*")->from("product_purchase")->where("supplier_id", $id)->having("due_amount >", 0)->order_by("purchase_date", "asc")->get()->result_array();
        ...
        if (empty($data)) {
            ...
            foreach ($data as $voucher) {
                $html .=
                    "<option value=" .  $voucher["purchase_id"] .  ">" .  $voucher["chalan_no"] .  "</option>";
            }
    ...
    public function insert_purchase_due( $purchase_id = null, $dbtid = null, $amnt_type = null, $amnt = null, $Narration = null, $Comment = null, $reVID = null, $subcode = null ) {
        ...
        $vaucherNo = "DV-" . ($maxid + 1);
        $debitinsert = [
            "fyear" => $fyear, "VNo" => $vaucherNo, "Vtype" => "DV", "referenceNo" => $purchase_id, "VDate" => $VDate, "COAID" => $dbtid, "Narration" => $Narration, "ledgerComment" => $Comment, "RevCodde" => $reVID, "subType" => 4, "subCode" => $subcode, "isApproved" => 0, "CreateBy" => $CreateBy, "CreateDate" => $createdate, "status" => 0, ];
    ...
    {
        return $this->db->select("acc_vaucher.*, SUM(acc_vaucher.Credit) as sumcredit, invoice.invoice" )->from("acc_vaucher")->join("invoice", "acc_vaucher.referenceNo=invoice.invoice_id", "left" )->where("acc_vaucher.referenceNo", $voucher_no)->where("acc_vaucher.CreateDate", $receive_data)->get()->result_array();
    }

    public function customerservicereceiptinfo($voucher_no, $date)
    {
        return $this->db->select("acc_vaucher.*, SUM(acc_vaucher.Credit) as sumcredit, service_invoice.voucher_no" )->from("acc_vaucher")->join("service_invoice", "acc_vaucher.referenceNo=service_invoice.voucher_no", "left" )->where("acc_vaucher.referenceNo", $voucher_no)->where("acc_vaucher.CreateDate", $date)->get()->result_array();
    }
    ...
    public function customer_service_due_vouchers($id)
    {
        ...
        $data = $this->db->select("*")->from("service_invoice")->where("customer_id", $id)->having("due_amount >", 0)->order_by("date", "asc")->get()->result_array();
