...
class Returns extends MX_Controller
{
    ...
    public function cloudsubset_return_form()
    {
    ...
    public function cloudsubset_invoice_return_form()
    {
        
        $invoice_id = trim($this->input->post('invoice_id', TRUE));
        $invid      = $this->db->select('inv_id')->from('invoice')->where('inv_id', $invoice_id)->get()->row();
        $query      = $this->db->select('inv_id')->from('invoice')->where('inv_id', $invoice_id)->get();
        $query2     = $this->db->select('inv_id')->from('product_return')->where('inv_id', $invoice_id)->get();
        
        if ($query->num_rows() == 0) {
            $this->session->set_flashdata(array(
                'exception' => display('please_input_correct_invoice_no')
            ));
        ...
        if ($query2->num_rows() > 0) {
            $this->session->set_flashdata(array(
                'exception' => display('already_returned')
            ));
            redirect('return_form');
        }
    ...
    public function cloudsubset_supplier_return()
    {
        ...
        if ($query->num_rows() == 0) {
            $this->session->set_flashdata(array(
                'exception' => display('please_input_correct_purchase_id')
            ));
    ...
    public function cloudsubset_invoice_return_list()
    {
