...
class Return_model extends CI_Model
{
    ...
    public function retrieve_invoice_html_data($return_id)
    {
        ...
        a.invoice_discount,a.total_discount,a.shipping_cost,a.prevous_due,a.total_amount,a.sales_by,a.invoice,a.policy_no,e.*,
        e.ret_invoice_id as invoice_id,a.returnable_amount');
        $this->db->from('product_return c');
        $this->db->join('invoice a', 'a.inv_id = c.invoice_id');
    ...
    public function return_invoice_entry()
    {
        ...
        if($this->input->post('product_id')!= '')
        {
            ...
            if($this->input->post('gross_commission_amount') > 0) {
                ...
                if($this->input->post('salesman_id') != '' && $this->input->post('salesman_id')!=0 && $this->input->post('salesman_commission_amount')>0){
                    $supp_prd = array(
                        'invoice_id' => $dbinv_id,
                        'type'=> 5,
                        'product_id' => $this->input->post('product_id', true),
                    ...
                    $salesman_product =$this->db->insert('salesman_product', $supp_prd);
                ...
                if($this->input->post('broker_id') != '' && $this->input->post('broker_id')!=0){
                    ...
                    $broker_product = $this->db->insert('broker_product', $broker_prd);
            ...
            $returns = array(
                ...
                'date_return' => $date,
                'product_rate' => $this->input->post('total_premium_amount_t', TRUE),
                // 'deduction' => $discount,
                // 'total_deduct' => $this->input->post('total_discountret', TRUE),
                'total_tax' => '',
                'total_ret_amount' => $this->input->post('total_premium_amount', TRUE),
                // 'net_total_amount' => $this->input->post('total_premium_amount_t', TRUE),
                'reason' => $this->input->post('inva_details', TRUE)
                // 'usablity' => $this->input->post('radio', TRUE)
            );
            
            $product_return = $this->db->insert('product_return', $returns);
        }
        ...
        $ret_adjust_amnt = array(
            'returnable_amount' => $this->input->post('grand_total_price', TRUE),
            'endorsement_no' => $this->input->post('endorsement_no', TRUE),
            'return_debit_note_no' => $this->input->post('return_debit_note_no', TRUE),
            'return_credit_note_no' => $this->input->post('return_credit_note_no', TRUE),
            'ret_adjust_amnt' => $paidamount,
            'invoice_type' => 5
        );
        $this->db->where('inv_id', $invoice_id)->update('invoice', $ret_adjust_amnt);
        ...
        if ($this->input->post('paid_amount') > 0) {
            ...
            //     $amount_pay = $multipayamount[$i];
                $COAID ='';
                $this->insert_sale_creditvoucher(0, $return_id, $COAID, $amnt_type, $this->input->post('paid_amount'), $Narration, $Comment, $reVID);
    ...
    {
        $this->db->where('invoice_id', $invoice_id)->delete('product_return');
    ...
    {
        $this->db->where('purchase_id', $purchase_id)->delete('product_return');
