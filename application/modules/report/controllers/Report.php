...
class Report extends MX_Controller
{
    ...
    /*product stock part*/
    function cloudsubset_stock_report()
    {
    ...
    public function cloudsubset_checkStocklist()
    {
        // GET data
        $postData = $this->input->post();
        $data = $this->report_model->cloudsubset_getStock($postData);
        echo json_encode($data);
    }

    public function cloudsubset_cash_closing()
    {
    ...
    public function add_daily_closing()
    {
        ...
        if ($closedata > 0)
        {
            $this->session->set_flashdata(array(
                'exception' => 'Already Closed Today'
            ));
    ...
    public function cloudsubset_closing_report()
    {
    ...
    public function cloudsubset_closing_report_search()
    {
    ...
    public function cloudsubset_todays_report()
    {
    ...
    public function cloudsubset_todays_customer_received()
    {
        $today = date('Y-m-d');
    ...
    //    ============ its for todays_customer_receipt =============
    public function cloudsubset_customerwise_received()
    {
    ...
    public function cloudsubset_todays_sales_report()
    {
        $sales_report = $this->report_model->todays_sales_report();
        $sales_amount = 0;
        $data = array(
    ...
    public function cloudsubset_datewise_sales_report()
    {
        $from_date = $this->input->get('from_date');
        $to_date = $this->input->get('to_date');
    ...
    public function cloudsubset_userwise_sales_report()
    {
    ...
    public function cloudsubset_invoice_wise_due_report()
    {
    ...
    public function cloudsubset_shippingcost_report()
    {
        $from_date = (!empty($this->input->get('from_date')) ? $this->input->get('from_date') : date('Y-m-d'));
        $to_date = (!empty($this->input->get('to_date')) ? $this->input->get('to_date') : date('Y-m-d'));
    ...
    public function cloudsubset_purchase_report()
    {
        $from_date = (!empty($this->input->get('from_date')) ? $this->input->get('from_date') : date('Y-m-d'));
        $to_date = (!empty($this->input->get('to_date')) ? $this->input->get('to_date') : date('Y-m-d'));
    ...
    public function cloudsubset_purchase_report_category_wise()
    {
        $from_date = (!empty($this->input->get('from_date')) ? $this->input->get('from_date') : date('Y-m-d'));
        $to_date = (!empty($this->input->get('to_date')) ? $this->input->get('to_date') : date('Y-m-d'));
    ...
    public function cloudsubset_sale_report_productwise()
    {
        $from_date = (!empty($this->input->get('from_date')) ? $this->input->get('from_date') : date('Y-m-d'));
        $to_date = (!empty($this->input->get('to_date')) ? $this->input->get('to_date') : date('Y-m-d'));
    ...
    public function cloudsubset_categorywise_sales_report()
    {
        $from_date = (!empty($this->input->get('from_date')) ? $this->input->get('from_date') : date('Y-m-d'));
        $to_date = (!empty($this->input->get('to_date')) ? $this->input->get('to_date') : date('Y-m-d'));
        $category = (!empty($this->input->get('category')) ? $this->input->get('category') : '');
    ...
    public function cloudsubset_sales_return()
    {
        $from_date = $this->input->post('from_date', true);
    ...
    public function cloudsubset_supplier_return()
    {
        $from_date = $this->input->post('from_date', true);
        $to_date = $this->input->post('to_date', true);
        $start = (!empty($from_date) ? $from_date : date('Y-m-d'));
    ...
    public function cloudsubset_tax_report()
    {
        $from_date = (!empty($this->input->get('from_date')) ? $this->input->get('from_date') : date('Y-m-d'));
        $to_date = (!empty($this->input->get('to_date')) ? $this->input->get('to_date') : date('Y-m-d'));
    ...
    public function cloudsubset_profit_report()
    {
    ...
    public function cloudsubset_add_closing()
    {
    ...
    public function cloudsubset_todays_commission_report()
    {
        $commission_report = $this->report_model->todays_commission_report();
        $sales_amount = 0;
        $data = array(
            'title' => display('commission_report') ,
            'sales_amount' => number_format($sales_amount, 2, '.', ',') ,
        );
        $data['module'] = "report";
        $data['page'] = "commission_report";
        echo modules::run('template/layout', $data);
    }

    public function cloudsubset_datewise_commission_report()
    {
        $from_date = $this->input->get('from_date');
        $to_date = $this->input->get('to_date');
        $commission_report = $this->report_model->retrieve_dateWise_CommissionReports($from_date, $to_date);
        $sales_amount = 0;
        if (!empty($commission_report))
        {
            ...
            foreach ($commission_report as $k => $v)
            {
                ...
                $commission_report[$k]['sales_date'] = $this->occational->dateConvert($commission_report[$k]['date']);
                $sales_amount = $sales_amount + $commission_report[$k]['total_amount'];
            }
        }
        $data = array(
            'title' => display('commission_report') ,
    ...
    public function getCommissionReportList()
    {
        ...
        // exit;
        $postData = $this->input->post();
        $data = $this->report_model->getCommissionReportList($postData);
        // dd($data);
        // exit;
        echo json_encode($data);
    }
    
    public function cloudsubset_todays_aggregator_report()
    {
        $sales_report = $this->report_model->todays_sales_report();
        $sales_amount = 0;
        $data = array(
            'title' => display('aggregator_report') ,
            'sales_amount' => number_format($sales_amount, 2, '.', ',') ,
        );
        $data['module'] = "report";
        $data['page'] = "aggregator_report";
        echo modules::run('template/layout', $data);
    }

    public function cloudsubset_datewise_aggregator_report()
    {
        $from_date = $this->input->get('from_date');
        $to_date = $this->input->get('to_date');
        $sales_report = $this->report_model->retrieve_dateWise_SalesReports($from_date, $to_date);
    ...
    public function cloudsubset_todays_broker_report()
    {
        $sales_report = $this->report_model->todays_sales_report();
        $sales_amount = 0;
        $data = array(
            'title' => display('broker_report') ,
            'sales_amount' => number_format($sales_amount, 2, '.', ',') ,
        );
        $data['referrals'] = $this->report_model->salesman_list();
        $data['brokers'] = $this->report_model->broker_list();
        $data['module'] = "report";
        $data['page'] = "broker_report";
        echo modules::run('template/layout', $data);
    }

    public function cloudsubset_datewise_broker_report()
    {
        $from_date = $this->input->get('from_date');
        $to_date = $this->input->get('to_date');
        $sales_report = $this->report_model->retrieve_dateWise_SalesReports($from_date, $to_date);
    ...
    public function cloudsubset_todays_referral_report()
    {
        $sales_report = $this->report_model->todays_sales_report();
        $sales_amount = 0;
        $data = array(
            'title' => display('referral_report') ,
            'sales_amount' => number_format($sales_amount, 2, '.', ',') ,
        );
        $data['referrals'] = $this->report_model->salesman_list();
        $data['brokers'] = $this->report_model->broker_list();
        $data['module'] = "report";
        $data['page'] = "referral_report";
        echo modules::run('template/layout', $data);
    }

    public function cloudsubset_datewise_referral_report()
    {
        $from_date = $this->input->get('from_date');
        $to_date = $this->input->get('to_date');
    ...
    public function cloudsubset_referral_report_productwise()
    {
        $from_date = (!empty($this->input->get('from_date')) ? $this->input->get('from_date') : date('Y-m-d'));
        $to_date = (!empty($this->input->get('to_date')) ? $this->input->get('to_date') : date('Y-m-d'));
        $product_id = (!empty($this->input->get('product_id')) ? $this->input->get('product_id') : '');
        ...
        $referral_list = $this->report_model->referral_list();
        if (!empty($product_report))
        {
            $i = 0;
            foreach ($product_report as $k => $v)
